...
export const GameConfig = memo(function GameConfig({
  ...
  const handleTimeSelect = useCallback((value: number) => { void updateConfig({ timeLimit: value }); }, [updateConfig]);
  const handleSpySelect = useCallback((count: number) => { void updateConfig({ spyCount: count }); }, [updateConfig]);
  const handleAutoStart = useCallback((checked: boolean) => { void updateConfig({ autoStartTimer: checked }); }, [updateConfig]);
  const handleHideSpy = useCallback((checked: boolean) => { void updateConfig({ hideSpyCount: checked }); }, [updateConfig]);
  const handleModerator = useCallback((checked: boolean) => { void updateConfig({ moderatorMode: checked }); }, [updateConfig]);
