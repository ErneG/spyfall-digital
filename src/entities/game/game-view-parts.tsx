...
import { AlertTriangle, Eye, EyeOff, MapPin, Pause, Play, Shield } from "lucide-react";
...
import type { GameView as GameViewData } from "./schema";
      ...
      <div className="flex gap-2 overflow-x-auto pb-1">
        {players.map((player) => (
          <div key={player.id} className="flex flex-col items-center gap-1">
            ...
            <span
              className={`text-[11px] ${
                player.id === currentPlayerId ? "text-spy-ink" : "text-muted-foreground"
              }`}
            >
              {player.name.length > NAME_TRUNCATE_LENGTH
                ? `${player.name.slice(0, NAME_TRUNCATE_LENGTH)}...`
                : player.name}
            </span>
          </div>
        ))}
