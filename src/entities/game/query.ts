...
import { getGameState } from "@/entities/game/actions";
...
export function useGameState(gameId: string | null, playerId: string | null) {
  const {
    ...
    queryFn: async () => {
      if (!gameId || !playerId) {
        throw new Error("Game and player are required");
      }

      return fetchGameState(gameId, playerId);
    },
